...
namespace Arts\Merlin\Includes;
...
class Downloader {
	...
	public function download_file( $url, $filename ) {
		...
		if ( ! empty( $saved_file ) ) {
			$file_path = $this->download_directory_path . $filename;
			do_action( 'merlin_downloader_file_downloaded', $file_path );
			return $file_path;
		}
	...
	private function get_content_from_url( $url ) {
		...
		if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ) {
			...
			return new \WP_Error(
				'download_error',
				sprintf(
					/* translators: 1: opening strong tag, 2: URL, 3: closing strong tag, 4: line break, 5: error code, 6: error message */
