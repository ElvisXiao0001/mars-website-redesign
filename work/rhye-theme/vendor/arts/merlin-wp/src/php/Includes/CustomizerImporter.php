...
class CustomizerImporter {
	...
	public static function import( $customizer_import_file_path ) {
		...
		Logger::get_instance()->info( __( 'The customizer import has finished successfully', 'merlin-wp' ) );
		return true;
	}
	 ...
	 * @return array|WP_Error An array of image data or WP_Error on failure.
	 */
	private static function customizer_sideload_image( $file ) {
		...
		if ( ! empty( $file ) ) {
			...
			if ( is_wp_error( $id ) ) {
				unlink( basename( realpath( $file_array['tmp_name'] ) ) );
				return $id;
			}
