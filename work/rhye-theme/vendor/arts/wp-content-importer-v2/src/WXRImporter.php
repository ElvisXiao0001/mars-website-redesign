...
class WXRImporter extends \WP_Importer {
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		if ( ! empty( $terms ) ) {
			...
			foreach ( $terms as $term ) {
				...
				$key      = hash( 'sha256', $taxonomy . ':' . $term['slug'] );
	...
	protected function process_term( $data, $meta ) {
		...
		$mapping_key = hash( 'sha256', $data['taxonomy'] . ':' . $data['slug'] );
	...
	protected function prefill_existing_comments() {
		...
		foreach ( $posts as $item ) {
			$exists_key                             = hash( 'sha256', $item->comment_author . ':' . $item->comment_date );
	...
	protected function comment_exists( $data ) {
		$exists_key = hash( 'sha256', $data['comment_author'] . ':' . $data['comment_date'] );
	...
	protected function prefill_existing_terms() {
		...
		foreach ( $terms as $item ) {
			$exists_key                          = hash( 'sha256', $item->taxonomy . ':' . $item->slug );
	...
	protected function term_exists( $data ) {
		$exists_key = hash( 'sha256', $data['taxonomy'] . ':' . $data['slug'] );
