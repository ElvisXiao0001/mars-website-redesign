 ...
 * @version   2.6.1 for a parent theme for publication on ThemeForest
...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		protected function do_plugin_install() {
			$plugin_get_param = isset( $_GET['plugin'] ) ? preg_replace('/[^-a-zA-Z0-9_]/', '', $_GET['plugin']) : '';

			if ( empty( $plugin_get_param ) ) {
				return false;
			}

			// All plugin information will be stored in an array for processing.
			$slug = $this->sanitize_key( urldecode( $plugin_get_param ) );
...
if ( ! function_exists( 'tgmpa' ) ) {
	...
	function tgmpa( $plugins, $config = array() ) {
		...
		/**
		 * Filters the plugins array before TGMPA registers them. Non-breaking — with no listener,
		 * `$plugins` is returned unchanged. Lets consumers (e.g. a sandbox/dev environment) rewrite
		 * per-plugin fields such as `source` URLs without touching each theme's load_plugins.php.
		 *
		 * @param array  $plugins    The plugins array passed to tgmpa().
		 * @param string $theme_slug The theme slug from $config['id'] (empty if not set).
		 */
		$plugins = apply_filters( 'arts/tgmpa/plugins', $plugins, isset( $config['id'] ) ? $config['id'] : '' );
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() || 'tgmpa-bulk-update' === $this->current_action() ) {
				...
				$plugin_post_param = isset( $_POST['plugin'] ) ? preg_replace('/[^-a-zA-Z0-9_]/', '', $_POST['plugin']) : '';

				// Did user actually select any plugins to install/update ?
				if ( empty( $plugin_post_param ) ) {
				...
				if ( is_array( $plugin_post_param ) ) {
					$plugins_to_install = (array) $plugin_post_param;
				} elseif ( is_string( $plugin_post_param ) ) {
					// Received via Filesystem page - un-flatten array (WP bug #19643).
					$plugins_to_install = explode( ',', $plugin_post_param );
				}
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				$plugins = array();
				if ( isset( $plugin_post_param ) ) {
					$plugins = array_map( 'urldecode', (array) $plugin_post_param );
